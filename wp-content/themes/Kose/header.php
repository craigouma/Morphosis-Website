...
<!--[if gt IE 8]><!--><html class="no-js" <?php language_attributes(); ?>><!--<![endif]-->
	<head>
        ...
        <title>Morphosis - Creative Space Architectural Design</title>
        <meta name="description" content="Morphosis is a multi displinary Architectural and Interior Design firm in Kenya with a focus of delivering quality, authentic and Creative Space.">
		...
		if($post_id==28){?>
        <style>
		#content-container{padding: 55px 0px 0px 216px;}#inner-content-container{padding-right: 0px !important;}
...
<script>
  ...
  ga('send', 'pageview');
	...
	<?php
		...
		if ( ! is_404() && ! is_search() && ! is_tax( 'uxbarn_portfolio_tax' ) ) {
			...
			if ( is_singular( 'uxbarn_portfolio' ) ) {
				...
				$slider_image_id2 = 'uxbarn_portfolio_slideshow_upload';
				$sliderback_id = 'uxbarn_portfolio_slideshow';
				//$slide_image_url = array();
			}
			//echo var_dump($slides);
				if ( is_singular( 'uxbarn_portfolio' ) ) {
				$slidebacks = uxbarn_get_array_value( get_post_meta( $post_id, $sliderback_id ), 0 );
				echo '<ul id="cbp-bislideshow" class="cbp-bislideshow">';
				foreach ( $slidebacks as $theslide ) {
				$slide_image_url = $theslide[ $slider_image_id2 ];
				echo '<li><img src="'.$slide_image_url.'" alt="'.$slide_image_url.'"/></li>';}
				echo '</ul>
	...
	?>
   	...
   	<script>
	function fbs_click(thelink,thetitle)
	{u=thelink;t=thetitle;window.open(	'http://www.facebook.com/sharer.php?u='+encodeURIComponent(u)+'&t='+encodeURIComponent(t),'sharer','toolbar=0,status=0,width=626,height=436');return false;}
	</script>
				...
				foreach ( $slides as $slide ) {
					...
					if ( $caption_display == 'on' ) {
						...
						if ( $title_tag != '' || $caption_text_tag != '' ) {
							...
							$caption_tag = 
								'<div class="slide-caption-wrapper ' . $caption_style_class_name . '">
								<div class="opclose"><img class="translated" src="'.get_template_directory_uri().'/images/plusw.png" width="10"></div>
								<div class="slide-caption" ' . $border_color_style . '>
	...
	<div id="root-container">
		<div id="inner-container">
			...
			<div id="side-container">
				...
				<div id="side-footer-wrapper" class="forwide">
